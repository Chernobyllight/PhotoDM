...
def custom_scale(image, scale_factor=0.5):
    width, height = image.size
    new_width = int(width * scale_factor)
    new_height = int(height * scale_factor)
    return image.resize((new_width, new_height), Image.BILINEAR)

def val_style_loader(args):
    val_trans = transforms.Compose([
                    # transforms.Resize(512),                   
                    # transforms.CenterCrop(512),
                    transforms.Lambda(lambda x: custom_scale(x, scale_factor=0.5)) , # 缩放到50%,
                    transforms.ToTensor()
                  ])

    val_dataset = ImageDataset(args.val_list_style, transform=val_trans)      
    val_loader = torch.utils.data.DataLoader(
                    val_dataset,
                    shuffle=False,
                    batch_size=args.batch_size,
...
def val_content_loader(args):
    val_trans = transforms.Compose([
                    # transforms.Resize(512),                   
                    # transforms.CenterCrop(512),
                    transforms.Lambda(lambda x: custom_scale(x, scale_factor=args.scale)) , # 缩放到50%,
                    transforms.ToTensor()
                  ])
    val_dataset = ImageDataset(args.val_list_content, transform=val_trans)      
    val_loader = torch.utils.data.DataLoader(
                    val_dataset,
                    shuffle=False,
                    batch_size=args.batch_size,
